...
import Test.Hydra.Prelude hiding (shouldBe, shouldReturn)
...
import Test.Util (shouldBe, shouldReturn)
...
withTestAPIServer port actor persistence tracer =
  withAPIServer defaultEnv "127.0.0.1" port actor persistence tracer dummyChainHandle defaultPParams noop
 where
  defaultEnv =
    Environment
      { party = actor
      ...
      , otherParties = []
      , contestationPeriod = Options.defaultContestationPeriod
