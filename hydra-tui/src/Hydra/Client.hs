...
import Hydra.API.HTTPServer (DraftCommitTxRequest (..), DraftCommitTxResponse (..), TxOutWithWitness (TxOutWithWitness))
   ...
   where
    request =
      Req.req
        ...
        (Req.ReqBodyJson . SimpleCommitRequest $ (`TxOutWithWitness` Nothing) <$> payload)
        Req.jsonResponse
